...
include_once '../DAO/tiexDAO.php';
include_once 'helper_controller/render_table.php';

class tiexController extends tiexDAO
{
    ...
    public function getTablaTiex($filtro, $pkID_proyectoM)
    {
        ...
        $array_opciones = [
            "modulo" => "tiex", //nombre del modulo definido para jquerycontrollerV2
            "title"  => "Click Ver Detalles", //etiqueta html title
            "href"   => "detalles_tiex.php?id_tiex=",
            "class"  => "detail", //clase que permite que añadir el evento jquery click
        ];
        //---------------------------------------------------------------------------------
        //carga el array desde el DAO
        if ($filtro == "'Todos'") {
            $grupo = $this->getTiexs($pkID_proyectoM);
        } else {
            $grupo = $this->getTiex($filtro, $pkID_proyectoM);
        }
    ...
    public function getTablaTiexEstudiantes($pkid_tiex)
    {
        ...
        //---------------------------------------------------------------------------------
        //carga el array desde el DAO
        $grupo = $this->getTiexEstudiantes($pkid_tiex);
        //print_r($grupo);
    ...
    public function getTablasesiones($pkID_tiex)
    {

        //$sesion = $this->getsesiones(pkID_tiex);
        $this->sesion = $this->getSesiones($pkID_tiex);
            ...
            for ($a = 0; $a < sizeof($this->sesion); $a++) {
                ...
                echo '
                             <tr>
                                 ...
                                 <td>
                                     <button id="edita_sesion" title="Editar" name="edita_sesion" type="button" class="btn btn-warning" data-toggle="modal" data-target="#frm_modal_tiex_sesion" data-id-sesion = "' . $id . '" ';
