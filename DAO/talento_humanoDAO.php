...
class talento_humanoDAO
{
    ...
    public function getFuncionariosCargo($pkID_proyectoM, $filtro)
    {
        if ($filtro == "'Todos'") {
            $where_anio = '';
        } else {
            $where_anio = "AND anio_funcionario_cargo = " . $filtro;
        }
        ...
        $this->q_general = "SELECT fkID_proyecto_marco,funcionario_cargo.pkID AS pkID,nombre_funcionario,apellido_funcionario,nombre_cargo,anio_funcionario_cargo,estado_funcionario_cargo FROM `funcionario_cargo`
            ...
            WHERE funcionario_cargo.estadoV = 1 " . $where_anio . " AND funcionario_cargo.fkID_proyecto_marco = " . $pkID_proyectoM;
